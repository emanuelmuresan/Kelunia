...
// TEMP diagnostic: paints a tiny badge with the native App Check outcome so it can
// be read off the screen on a device without USB debugging. Remove once native
// App Check is confirmed working end to end.
function keluniaAppCheckDiag(message) {
  try {
    window.__keluniaAppCheckDebug = message;
    console.log("KELUNIA_APPCHECK_DIAG:", message);
    const render = () => {
      let el = document.getElementById("kelunia-appcheck-diag");
      if (!el) {
        el = document.createElement("div");
        el.id = "kelunia-appcheck-diag";
        el.style.cssText =
          "position:fixed;left:0;right:0;bottom:0;z-index:2147483647;background:#111;color:#0f0;" +
          "font:11px/1.4 monospace;padding:4px 8px;white-space:pre-wrap;word-break:break-word;opacity:.92";
        el.addEventListener("click", () => el.remove());
        document.body.appendChild(el);
      }
      el.textContent = "AppCheck: " + message + "  (tap to dismiss)";
    };
    if (document.body) {
      render();
    } else {
  ...
  if (nativeShell) {
    ...
    import("@capacitor-firebase/app-check")
      .then(async ({ FirebaseAppCheck }) => {
        ...
        try {
          const firstToken = await getAppCheckToken(appCheckInstance);
          keluniaAppCheckDiag(`OK token len=${firstToken?.token?.length ?? 0}`);
        } catch (error) {
          console.warn("Primul token App Check nu a putut fi obtinut:", error);
          keluniaAppCheckDiag(`FAIL getToken: ${String(error?.message || error)}`);
        }
      })
      .catch((error) => {
        console.warn("App Check nativ nu a putut fi inițializat:", error);
        keluniaAppCheckDiag(`FAIL init: ${String(error?.message || error)}`);
      })
      .finally(() => {
        resolveAppCheckReady();
      });
