...
import { browserLocalPersistence, connectAuthEmulator, getAuth, indexedDBLocalPersistence, initializeAuth, setPersistence } from "firebase/auth";
import {
...
// Local E2E / CI only: point Auth, Firestore and Functions at the Firebase
// emulators and skip App Check entirely. Never set NEXT_PUBLIC_FIREBASE_EMULATOR
// in a real deploy.
const useFirebaseEmulator =
  process.env.NEXT_PUBLIC_FIREBASE_EMULATOR === "1" && typeof window !== "undefined";
...
if (typeof window === "undefined" || useFirebaseEmulator) {
  // SSR/build (App Check only runs client-side) or emulator mode (no App Check):
  // resolve immediately so nothing that awaits this dangles.
  resolveAppCheckReady();
}

if (!useFirebaseEmulator && typeof window !== "undefined" && !window.__keluniaAppCheckInitialized) {
...
if (useFirebaseEmulator) {
  try {
    ...
    console.warn("Nu s-au putut conecta emulatoarele Firebase:", error);
  }
}
